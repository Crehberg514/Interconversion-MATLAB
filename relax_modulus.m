function modTime = relax_modulus(M0, MMats, timeConst, time)
    % Shortcut to compute the anisotropic relaxation modulus, at a given time.
    %
    % A shortcut function that presets the relaxation flag in the
    % modulus_at_time function. Calculates the creep modulus of an anisotropic
    % material function at a given time . Requires the equilibrium matrix,
    % coefficient matrices, the inverted time constants, and the time at which
    % the modulus will be calculated.
    %
    % Parameters
    % ----------
    % M0 : 2D matrix
    %     An equilibrium modulus in a 2D array.
    % MMats : 3D matrix
    ...
    modTime = modulus_at_time(M0, MMats, timeConst, time, 'relax');
